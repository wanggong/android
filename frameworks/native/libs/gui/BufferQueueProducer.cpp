    ...
    while (tryAgain) {
        ...
        *found = BufferQueueCore::INVALID_BUFFER_SLOT;
        ...
        if (tooManyBuffers) {
            ...
            //make sure framebuffer alloc 3, bug caused by 2d and gpu buffer sync.
            //warning: fix this bug on the platform lc1860, do not need modify on lc1861
            bool need_alloc = false;
            if (!mCore->mFreeSlots.empty()
                        && !strcmp("FramebufferSurface" ,mConsumerName.string())) {
                auto slot = mCore->mFreeSlots.begin();
                if (*slot < maxBufferCount) {
                    need_alloc = true;
                }
            }
            if (!mCore->mFreeBuffers.empty() && !need_alloc) {
