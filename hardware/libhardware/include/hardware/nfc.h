...
typedef struct nfc_nci_device {
    ...
    /* ******* start of extra methods required for BRCM extras ******** */

    /*
     * (*enter_ce_low_power_mode)() Notify HAL that NFCC is ready to enter
     * CE low power mode.
     */
    int (*enter_ce_low_power_mode)(const struct nfc_nci_device *p_dev);

    /*
     * (*enter_full_power_mode)() Notify HAL that NFCC is booting up to
     * full power mode.
     */
    int (*enter_full_power_mode)(const struct nfc_nci_device *p_dev);

    /*
     * (*update_snooze_mode)() Update Snooze mode;
     * HAL_NFC_BRCM_CMPL_EVT will be returned with status.
     */
    int (*update_snooze_mode)(const struct nfc_nci_device *p_dev,
        uint8_t snooze_mode, uint8_t idle_threshold_dh,
        uint8_t idle_threshold_nfcc, uint8_t nfc_wake_active_mode,
    ...
    /* ******** end of extra methods required for BRCM extras ********* */
